...
namespace yii3ds\promotions\controllers\backend;

use vova07\admin\components\Controller;
use yii3ds\promotions\models\backend\Product;
use yii3ds\promotions\models\backend\ProductSearch;
...
class DefaultController extends Controller
{
    ...
    public function behaviors()
    {
        ...
        $behaviors['access']['rules'][] = [
            'allow' => true,
            'actions' => ['create'],
            'roles' => ['BCreatePromotions']
        ];
        $behaviors['access']['rules'][] = [
            'allow' => true,
            'actions' => ['update'],
            'roles' => ['BUpdatePromotions']
        ];
        $behaviors['access']['rules'][] = [
            'allow' => true,
            'actions' => ['delete', 'batch-delete'],
            'roles' => ['BDeletePromotions']
        ];
        $behaviors['access']['rules'][] = [
            'allow' => true,
            'actions' => ['imperavi-get', 'imperavi-image-upload', 'imperavi-file-upload', 'fileapi-upload'],
            'roles' => ['BCreatePromotions', 'BUpdatePromotions']
        ];
        $behaviors['verbs'] = [
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                if ($model->save(false)) {
                    ...
                    Yii::$app->session->setFlash('danger', Module::t('promotions', 'BACKEND_FLASH_FAIL_ADMIN_CREATE'));
                    return $this->refresh();
                }
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                if ($model->save(false)) {
                    ...
                    Yii::$app->session->setFlash('danger', Module::t('promotions', 'BACKEND_FLASH_FAIL_ADMIN_UPDATE'));
                    return $this->refresh();
                }
            } elseif (Yii::$app->request->isAjax) {
                Yii::$app->response->format = Response::FORMAT_JSON;
     ...
     *
     * @return \yii3ds\promotions\models\backend\Product Model
     *
    ...
    {
        if (is_array($id)) {
            /** @var \yii3ds\promotions\models\backend\Product $model */
