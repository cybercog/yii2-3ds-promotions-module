...
namespace yii3ds\promotions\controllers\frontend;

use yii3ds\promotions\models\frontend\Product;
use yii3ds\promotions\components\Productcomponent;
use Yii;
...
class DefaultController extends Controller
{
    ...
    function actionIndex($id='')
    {
        ...
        $this->setCategory();
        return $this->getPromotionsResult($id,\yii::$app->request->get('language','th'));
    }
    ...
    function getPromotionsResult($id='',$language='')
    {
        $result = [];
        if ($this->isInputNotHavePromotionsID($id)) {
            $result = $this->getAllPromotionsData($language);
        }else{
            $result = $this->getPromotionsById($id, $language);
        }

        return $this->createResult($result, $language);
    }
    function isInputNotHavePromotionsID($id)
    {
        return empty($id);
    }
    ...
    function getAllPromotionsData($language)
    {
    ...
    function getPromotionsById($id='', $language)
    {
        $isNull = $this->category == null;
        if( $isNull )
        {
